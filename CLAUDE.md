...
`static/regestio/` is the **Regestio** landing, served at **`/regestio/`** (the canonical URL — `static/_redirects` 301s the old `/gestionale-corsi-associazioni/*` path here). It's the Unitre gestionale productized; the related portfolio entries are `content/portfolio/regestio.md` (the product itself) and `content/portfolio/unitre-gestionale.md` (the original build). The pricing section (`#prezzi`) offers monthly subscription plans: Small 99€ (≤300 soci), Medium 299€ (≤1.000), Large 499€ (≤2.000), Ultra on request (mailto). FAQ and JSON-LD carry the same prices — keep all three in sync when changing them.
...
Links from the main site to `/regestio/`: (1) its own **portfolio entry** `content/portfolio/regestio.md` (shows as a card in the portfolio grid like the other projects, with a branded cover `images/portfolio/regestio-feature.png`; its `projectURL` points to the landing); (2) the **Unitre case study** `content/portfolio/unitre-gestionale.md` — front-matter `productURL: /regestio/` renders a primary "Discover Regestio" button in `layouts/portfolio/single.html` (the `projectURL` button is `btn-outline-primary`), plus an inline link in the body. (There was briefly a dedicated homepage card — `regestioSection` partial/data — but it was removed in favor of the portfolio entry.)

Brand assets: `static/regestio/brand/` — `icon.svg` + `icon-*.png` (R mark, green tile) and `logo[.svg|.png]` / `logo-white.*` (icon + "Regestio" wordmark, light/dark bg). SVG masters have glyphs converted to paths (DM Serif Display, font-independent). Brand color `#2e8b57`, accent `#0e1f16`. Used for the favicon/apple-touch-icon/og:image and for Stripe branding (icon = `icon-512.png`, logo = `logo.png`).
